...
//dotenv.config();
const API_KEY = "WBUoCfaNeQDusmLJKNW3OAshmdefAfgzvBLf65cd";

// endpoint URL
const searchBaseURL = "https://api.si.edu/openaccess/api/v1.0/search";
// our search term
const search =  `place:"New York" AND unit_code:"NPG" AND online_media_type:"Images" AND NOT title:"G.W.M. Nutt and Minnie Warren" AND NOT title:"Charles and Lavinia Stratton" and NOT title:"Strattons, G.W.M. Nutt and Minnie Warren (wedding party)" and NOT title:"Possibly Pauline Cushman"`
...
function fetchUrl(searchAllURL){
  request(searchAllURL, function (error, response, body) {
    ...
    let objects = obj.response.rows.filter(data => {
      ...
      if(data.content.indexedStructured.date ==undefined || data.content.freetext.name==undefined)dataComplete=false;
...
let currentPlace = "";
  if(data.content.indexedStructured.place) {
    currentPlace = data.content.indexedStructured.place[0];
  }

let currentDate = "";
  if(data.content.freetext.date) {
    currentDate = data.content.freetext.date[0].content;
  }
  
let portraitPerson="";
if(data.content.freetext.name[1]) {
    portraitPerson = data.content.freetext.name[1].content;
  }
      return { 
        objectID: data.id,
        title: data.title,
        portrait: portraitPerson,
        place: currentPlace,
        date: currentDate,
